      ...
      {players.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {players.map((p) => (
            <div
              key={p.id}
              className="bg-panel-2/40 p-5 rounded-2xl border border-slate-700/50 flex justify-between items-center group hover:border-cyan-500/50 transition-all duration-300 hover:bg-panel-2/80 backdrop-blur-sm"
            >
              {editingId === p.id ? (
                <input
                  type="text"
                ...
                <span className="font-black text-slate-200 text-lg pl-2 tracking-wider group-hover:text-cyan-100 transition-colors">
                  {p.name}
                </span>
              )}
